//  END CODE PROVIDED BY GONG
// ==========================================================================================================================

import { getUserBySecret, type User } from "../services/user-service";
...
/**
 * Logs the user into the dashboard, used by the login form.
 * @param email The users email input from the login form
 * @param password The users input password
 * @returns {user, secret} The user (or null if not found) secret to store for later
 */
export async function authenticateUser(
  email: string,
  password: string
): Promise<{ user: User | null; secret: string }> {
  const encodedUserString = encode(email, password);
  // FETCH USER ID FROM JSON
  const user = await getUserBySecret(encodedUserString);
  // IF VALID LOGIN
  if (user) {
    return { user, secret: encodedUserString };
  } else {
    // IF NULL
...
/**
 * Log the user in with a given secret
 * @param secret The users secret
 * @returns User or null
 */
export async function authenticateUserWithSecret(
  secret: string
): Promise<User | null> {
  // FETCH USER ID FROM JSON
